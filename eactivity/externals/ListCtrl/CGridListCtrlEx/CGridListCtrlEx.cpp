...
void CGridListCtrlEx::SetTextColor(int row, COLORREF textColor)
{
	if (row >= clrsText.GetSize())
	{
		clrsText.SetSize(row+1);
	}
...
{
	if (clrsText.GetSize()>nRow && clrsText[nRow]>0 && nCol==1)
	{
...
//!
//! @return Number of header control items if successful; otherwise Ц 1.
//------------------------------------------------------------------------
