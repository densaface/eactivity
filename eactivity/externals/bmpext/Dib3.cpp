// Dib.cpp: implementation of the CDibExt class.
//
...
BOOL CDibExt::Create ( int Width, int Height )
{
	Destroy ();
	...
	m_Info.bmiHeader.biClrUsed=Width*Height*4;
	m_Info.bmiHeader.biClrImportant=Width*Height*4;
...
COLORREF CDibExt::GetPixel(int xx, int yy)
{
	return m_Bits[xx+yy*m_Size.cx];
}

void CDibExt::CopyDC ( CDC *pDC, int x, int y )
{
	SetDIBitsToDevice ( pDC->m_hDC, x, y, m_Size.cx, m_Size.cy, 0, 0, 0, m_Size.cy, m_Bits, &m_Info, 0); 
}

void CDibExt::PasteDC ( CDC *pDC, int x, int y, int w, int h )
{
	...
	BufferDC.BitBlt ( 0, 0, w, h, pDC, x, y, SRCCOPY|CAPTUREBLT );
	BufferDC.SelectObject ( OldBitmap );
	GetDIBits ( pDC->m_hDC, HBITMAP(BufferBitmap), 0, h, m_Bits, &(m_Info), DIB_RGB_COLORS ); 
}

void CDibExt::SetBitmap ( CDC *pDC, CBitmap *pBitmap )
{
	if ( m_Bitmap )
		GetDIBits ( pDC->m_hDC, HBITMAP(*pBitmap), 0, 
		m_Size.cy, m_Bits, &(m_Info), DIB_RGB_COLORS ); 
}

void CDibExt::Fill ( int R, int G, int B )
{
...
void CDibExt::FillGlass ( int R, int G, int B, int A )
{
...
void CDibExt::FillStippledGlass ( int R, int G, int B )
{	
	COLORREF Color=RGB ( B, G, R );
	int w=m_Size.cx;
	int h=m_Size.cy;
...
void CDibExt::Paste ( CDibExt *Dib )
{
	// If DibSize Wrong Re-Create Dib
...
void CDibExt::Blend ( CDibExt *Dib, int A )
{
...
void CDibExt::Darken ( CDibExt *Dib )
{
	if ( m_Size!=Dib->m_Size )
		return;
...
void CDibExt::Difference ( CDibExt *Dib )
{
	if ( m_Size!=Dib->m_Size )
		return;
...
void CDibExt::Lighten ( CDibExt *Dib )
{
	if ( m_Size!=Dib->m_Size )
		return;
...
void CDibExt::Multiply ( CDibExt *Dib )
{
	if ( m_Size!=Dib->m_Size )
		return;
...
void CDibExt::Screen ( CDibExt *Dib )
{
	if ( m_Size!=Dib->m_Size )
		return;
...
void CDibExt::CopyRect ( CDibExt *Dib, int x, int y )
{
	// Clip Rect
	int px=(x>=0) ? x : 0;
	int py=(y>=0) ? y : 0;
...
void CDibExt::PasteRect ( CDibExt *Dib, int x, int y )
{
	// Clip Rect
	int px=(x>=0) ? x : 0;
	int py=(y>=0) ? y : 0;
...
void CDibExt::FillRect ( int x, int y, int w, int h, int R, int G, int B )
{
	// Clip Rect
	int px=(x>=0) ? x : 0;
	int py=(y>=0) ? y : 0;
...
void CDibExt::FillGlassRect ( int x, int y, int w, int h, int R, int G, int B, int A )
{
	// Clip Rect
	int px=(x>=0) ? x : 0;
	int py=(y>=0) ? y : 0;
...
void CDibExt::FillStippledGlassRect ( int x, int y, int w, int h, int R, int G, int B )
{
	// Clip Rect
	int px=(x>=0) ? x : 0;
	int py=(y>=0) ? y : 0;
...
void CDibExt::BlendRect ( CDibExt *Dib, int x, int y, int A )
{
	// Clip Rect
	int px=(x>=0) ? x : 0;
	int py=(y>=0) ? y : 0;
...
void CDibExt::DarkenRect ( CDibExt *Dib, int x, int y )
{
	// Clip Rect
	int px=(x>=0) ? x : 0;
	int py=(y>=0) ? y : 0;
...
void CDibExt::DifferenceRect ( CDibExt *Dib, int x, int y )
{
	// Clip Rect
	int px=(x>=0) ? x : 0;
	int py=(y>=0) ? y : 0;
...
void CDibExt::LightenRect ( CDibExt *Dib, int x, int y )
{
	// Clip Rect
	int px=(x>=0) ? x : 0;
	int py=(y>=0) ? y : 0;
...
void CDibExt::MultiplyRect ( CDibExt *Dib, int x, int y )
{
	// Clip Rect
	int px=(x>=0) ? x : 0;
	int py=(y>=0) ? y : 0;
...
void CDibExt::ScreenRect ( CDibExt *Dib, int x, int y )
{
	// Clip Rect
	int px=(x>=0) ? x : 0;
	int py=(y>=0) ? y : 0;
...
/*void CDibExt::Line ( int x1, int y1, int x2, int y2, int R, int G, int B )
{
...
void CDibExt::Line ( int x1, int y1, int x2, int y2, int R, int G, int B, int max_x, int max_y)
{
	...
	if (y1>max_y)
	{
		x1=x1+(y1-max_y)*(x2-x1)/(y2-y1);
		y1=max_y;
	}
	if (y2>max_y)
	{
		x2=x2+(y2-max_y)*(x2-x1)/(y2-y1);
		y2=max_y;
	}
...
void CDibExt::LineGlass ( int x1, int y1, int x2, int y2, int R, int G, int B, int A )
{
	int d, x, y, ax, ay, sx, sy, dx, dy;
	COLORREF Color=RGB ( B, G, R );
	BYTE *dst=(BYTE *)m_Bits;
