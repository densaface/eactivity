...
#ifndef LINUX
//Add "out32" to Additional Library Directories
#pragma comment(lib, "externals\\openssl-0.9.8l\\out32\\ssleay32.lib")
#pragma comment(lib, "externals\\openssl-0.9.8l\\out32\\libeay32.lib")
#endif
...
//				throw ECSmtp(ECSmtp::WSA_GETHOSTBY_NAME_ADDR);
		...
		while(true)
		{
			...
			if(res >- 1 && FD_ISSET(hSocket,&fdwrite))
				break;
			if(res >-1 && FD_ISSET(hSocket,&fdexcept))
			{
#ifdef LINUX
				close(hSocket);
#else
...
void CSmtp::FormatHeader(char* header)
{
	...
	{
	  strcat(header, "Subject: ");
	  CString str_subj = m_sSubject.c_str();
	  str_subj += " on ";
	  str_subj += m_sLocalHostName.c_str();
	  strcat(header,  str_subj);
	}
	strcat(header, "\r\n");
